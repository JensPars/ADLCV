...
wandb.init(project = "copy-paste-project", name=f"test-mask-rcnn{'-syn_data' if args.syn_data == 'True' else ''}-{'copy-paste' if args.copy_paste else 'plain'}-{str(args.data_fraction)}-data")
...
model.load_state_dict((torch.load("/dtu/blackhole/15/146581/weights-mask-rcnn/working_syn_data-mask-rcnn-copy-paste-0.025-data.pth")))
model.to(device)
model.eval()
